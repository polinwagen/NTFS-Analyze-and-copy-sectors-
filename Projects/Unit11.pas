...
begin
  ...
  sa:=Pansichar('\\.\'+form11.DriveCombobox1.Drive+':');
   ...
   if s=('\\.\c:')
then begin
  MessageBox(0, 'Выбор системного диска для анализа-запрещен', 'Ошибка', MB_ICONWARNING or MB_OK);
  exit;
  end;
...
then begin
  MessageBox(0, 'Файловая система FAT32 не поддерживается!', 'Ошибка', MB_ICONWARNING or MB_OK);
...
//считаем всю mft
 ...
 if select[0]='' then
 begin
 MessageBox(0, 'Не выбран ни один файл', 'Ошибка', MB_ICONWARNING or MB_OK);
  exit;
 end;
